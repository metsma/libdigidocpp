...
const DataFile* findDataFile(const char *name)
{
    if(!cb_doc)
        return {};
    ...
    if(const DataFile *file = find(cb_doc->dataFiles()))
        return file;
    if(cb_doc->mediaType() == ASiC_E::MIMETYPE_ADOC)
        return find(static_cast<ASiC_E*>(cb_doc)->metaFiles());
    return {};
}
...
int initXmlSecCallback()
{
    xmlSecErrorsSetCallback([](const char *file, int line, const char *func,
        ...
        if(cb_exception)
            cb_exception->addCause(Exception(orUnknown(file), line, Log::format("%s:obj=%s:subj=%s:reason=%d - %s %s",
    ...
    return xmlSecIORegisterCallbacks(
        [](const char *name) -> int {
            return findDataFile(name) ? 1 : 0;
        },
        [](const char *name) -> void * {
            if(const DataFile *file = findDataFile(name))
            {
...
void SignatureXAdES_B::validate(const string &policy) const
{
    ...
    bool result = XMLDocument::verifySignature(signature);
    cb_doc = {};
    cb_exception = {};
    if(!result)
